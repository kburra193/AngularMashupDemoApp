...
import { DEFAULT_SORT_ASCII, SortCriteria } from "../../data/types/sort-criteria-options";
...
export class FiltersComponent {
  ...
  async ngOnInit() {
    ...
    try {
      [this.regionData, this.yearData, this.quarterData, this.monthData, this.datesData] = await Promise.all([
  ...
  async selectValueByField(fieldName: string, fieldValue: any, isNumeric: boolean, alternateState: AlternateState = AlternateState.DEFAULT){
    fieldName === 'Year' ? this.selectedYear = fieldValue : this.selectedRegion = fieldValue;
    //collect field values to select
    let selectedValues = !isNumeric ? [fieldValue].map(selectedOption => ({ qText: selectedOption })) :
                                      [fieldValue].map(selectedOption => ({ qIsNumeric: true, qNumber: selectedOption}));
    //apply selected values by field
    await this.qlikService.selectMultipleValuesPerField(fieldName, selectedValues, alternateState);
    //Refresh data
    await this.refreshData();
  }
  ...
  async onSelectionChange(fieldName: string, selectedValues: any, isNumeric: boolean){
    //apply values
    ...
    this.selectedQuarter = this.quarterData.filter(quarter => quarter[0].qState === 'S');
    // Set selectedMonth based on condition
    this.selectedMonth = this.monthData.filter(month => month[0].qState === 'S');
    //Refresh data
    await this.refreshData();
  }
  ...
  async getTableData(tableConfig: any){
    ...
    return await this.qlikService.getHyperCubeData(
      hyperCubeDef, numColumns, tableConfig.alternateState
    );
  }
  ...
  async onRegionDefaultChange(fieldName: string, selectedValues: any, isNumeric: boolean, alternateState: AlternateState = AlternateState.DEFAULT){
    //apply values
    await this.applySelectedValues(fieldName, selectedValues, isNumeric, alternateState);
    await this.refreshData();
  }

  async onRegionBdpChange(fieldName: string, selectedValues: any, isNumeric: boolean, alternateState: AlternateState = AlternateState.DEFAULT){
    //apply values
    await this.applySelectedValues(fieldName, selectedValues, isNumeric, alternateState);
    await this.refreshData();
  }
  ...
  private async applySelectedValues(fieldName: string, fieldValue: any, isNumeric: boolean, alternateState: AlternateState = AlternateState.DEFAULT) {
    if(fieldValue){
  ...
  private appendSheet(wb: XLSX.WorkBook, ws: XLSX.WorkSheet, sheetName: string) {
